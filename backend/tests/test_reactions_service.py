...
class TestReactionService:
    ...
    # Tests for P2-API-Service-Reactions-ThreadSave functionality
    @pytest.mark.asyncio
    async def test_react_thread_save_success(self):
        """Test successful thread save reaction."""
        # Mock repository
        from app.repositories.reactions_repo import ReactionRepository
        mock_repo = AsyncMock(spec=ReactionRepository)
        mock_repo.insert_save_if_absent = AsyncMock(return_value=True)  # New reaction inserted
        
        # Mock repository constructor
        with patch('app.services.reactions_service.ReactionRepository', return_value=mock_repo):
            result = await self.service.react_thread_save(
                user_id="usr_01H0000000000000000000000X",
                thread_id="thr_01H0000000000000000000000X"
            )
        ...
        # Verify repository was called with correct parameters
        mock_repo.insert_save_if_absent.assert_called_once_with(
            target_id="thr_01H0000000000000000000000X",
            user_id="usr_01H0000000000000000000000X"
        )
    
    @pytest.mark.asyncio
    async def test_react_thread_save_conflict(self):
        """Test thread save reaction when already exists (409 Conflict)."""
        # Mock repository
        from app.repositories.reactions_repo import ReactionRepository
        mock_repo = AsyncMock(spec=ReactionRepository)
        mock_repo.insert_save_if_absent = AsyncMock(return_value=False)  # Reaction already exists
        
        # Mock repository constructor
        with patch('app.services.reactions_service.ReactionRepository', return_value=mock_repo):
            with pytest.raises(ConflictException) as exc_info:
                await self.service.react_thread_save(
                    user_id="usr_01H0000000000000000000000X",
                    thread_id="thr_01H0000000000000000000000X"
                )
        ...
        # Verify repository was called
        mock_repo.insert_save_if_absent.assert_called_once()
    
    @pytest.mark.asyncio
    async def test_react_thread_save_invalid_thread_id_format(self):
        """Test thread save reaction with invalid thread ID format."""
        # Invalid format (not thr_*)
        with pytest.raises(ValidationException) as exc_info:
            await self.service.react_thread_save(
                user_id="usr_01H0000000000000000000000X",
    ...
    @pytest.mark.asyncio
    async def test_react_thread_save_malformed_thread_id(self):
        """Test thread save reaction with malformed thread ID."""
        # Correct prefix but wrong length
        with pytest.raises(ValidationException) as exc_info:
            await self.service.react_thread_save(
                user_id="usr_01H0000000000000000000000X",
